...
# One burn-in for the whole project.
BURN_IN = 60
DEV_END = "2015-12-31"
...
def walk_cov(r, rf, weight_fn, rates, labels=None, min_regime=24):
    ...
    for i in range(len(r)):
        ...
        if labels is not None:
            now = labels.iloc[i]                       # already lagged
            if pd.notna(now):
                same = labels.iloc[:i] == now
                sub = ex[same.reindex(ex.index).fillna(False).to_numpy()]
                # Only use the in-regime estimate once that regime has enough
                # of its own history to estimate from.
...
# Regime labels are built from industrial production and CPI, both of which are
# published with a delay. The label for month t is not knowable until the prints
# land, so it is lagged before any model is allowed to condition on it.
# Otherwise a model "knows" which state it is in before the data that defines
# the state exists, which is look-ahead however causally the label itself was
# constructed.
REGIME_LAG = 2
...
def tune_and_forecast(r, rf, X, assets, dev_mask):
    """
    Grid search on development only, then one forecast at the chosen
    setting.
    """
