...
# Display names for the result tables.
STRATEGY_NAME = {
    "HRP + Rolling 60m overlay": "HRP + 60m rolling adaptive momentum overlay",
    "Rolling 60m, long only": "60m rolling adaptive momentum",
    "Rolling 60m (1m target), long only":
        "60m rolling adaptive momentum (1m target)",
    "Momentum (Sharpe), long only": "Momentum (Sharpe)",
    "Momentum 12-1, long only": "Momentum 12-1",
    "Regression, base": "Regression",
    "ML ridge, base": "ML ridge",
    "ML elastic net, base": "ML elastic net",
    "ML gradient boosting, base": "ML gradient boosting",
    "ML random forest, base": "ML random forest",
}
...
def result_table(r, df, caption, status=False):
    ...
    t = t.rename(columns={
        ...
        "vs_agg": "vs the Agg", "p": "p", "alpha_pct_yr": "alpha, %",
...
def phase2():
    ...
    r.prose(
        ...
        "already was. The 60m rolling adaptive momentum overlay moves further and adds 0.10 "
    ...
    r.prose(
        "<strong>Every model that estimates a return ends up taking more market "
        "risk than the index.</strong> Their betas to the Aggregate run from "
        "1.21 for the elastic net to 2.03 for the combined regression, while "
        "the risk-based methods sit between 0.59 and 0.87. A signal weighted "
        "portfolio concentrates into whatever has been rising, and in a "
        "universe of bonds that is usually the long end, so each of them "
        "arrives at a levered position on the level of rates without ever "
        "having been asked for one.")
    r.prose(
        "Raw momentum shows it most plainly, at 7.6 years of duration against "
        "the index at 4.2 and a beta of 1.45. Duration is not reported for the "
        "regression and machine learning books: they trade on a forecast that "
        "moves every month, so a single average duration describes them poorly "
        "and their beta is the more honest summary of what they are exposed "
        "to.")
    ...
    r.prose(
        ...
        "case, and the 60m rolling adaptive momentum strategy and momentum "
...
def phase3():
    ...
    r.prose(
        ...
        "2022 against the index at 13.2%. The obvious explanation would be that "
        "they hold less rate risk, but they do not: hierarchical risk parity "
        "runs 4.1 years of duration against the index at 4.2, and equal risk "
        "contribution runs 5.1. What separates them is diversification across "
        "the curve and across credit rather than a shorter book.")
    ...
    if HW is not None and len(HW):
        ...
        r.prose(
            ...
            f"around {m['duration contribution'].sum():.1f} years, close to the "
            "index's 4.2, so the book is not short duration; it is holding the "
            "same rate exposure spread across more of the curve. The weights "
            "are also stable, which is why turnover is low.")
    ...
    r.prose(
        ...
        "run below the index on volatility. On duration they sit either side "
        "of it: hierarchical risk parity at 4.1 years against the index at "
        "4.2, and equal risk contribution at 5.1. Neither is reaching for "
        "return by extending duration.")
    r.prose(
    ...
    r.prose(
        "<strong>The 60m rolling adaptive momentum strategy is the one open "
...
def appendix_universe():
    ...
    if fipred is not None:
        t = fipred.copy()
        t["oos_r2"] = t["oos_r2"] * 100
        t["vol"] = t["vol"] * 100
        t["risk_share"] = t["risk_share"] * 100
        t.index = [ASSET_NAME.get(a, a) for a in t.index]
        t.columns = ["R squared, %", "volatility, %", "duration, years",
                     "share of universe risk, %"]
        t.index.name = "holding"
        r.table(t.round(2), compact=True, align_right=list(t.columns),
                heat=["R squared, %"],
                caption="Development sample, monthly. R squared is from the "
                        "univariate combination forecast. Share of universe "
                        "risk is each holding's variance as a fraction of the "
                        "total. Monthly rather than daily, because the daily "
                        "figures for the three matrix-priced funds are "
                        "inflated by the pricing lag.")
    r.prose(
        ...
        "with a rank correlation of -0.87, and it is the clearest pattern in "
        "the universe.</strong> The short "
...
def appendix_method():
    ...
    r.table(pd.DataFrame([
        ("Treasuries, 2 to 30 year", "3bp", "Repo or the futures basis. "
         "General collateral repo is lending against any Treasury the borrower "
         "chooses to deliver, so it is the cheapest secured borrowing that "
         "exists. SOFR is itself built from general collateral Treasury repo, "
         "which makes financing a Treasury close to flat to the reference "
         "rate."),
        ...
        caption="What each sleeve is assumed to cost to finance, and the "
    ...
    if REB is not None:
        ...
        t = REB.loc[keep, [c for c in ["dev_sharpe", "trades", "turnover"]
                           if c in REB.columns]]
        t.columns = ["dev Sharpe", "trades", "turnover"][:t.shape[1]]
        t.index.name = "strategy"
        r.table(t.round(4), compact=True, align_right=list(t.columns),
                caption="Development sample, expanding covariance window, all "
                        "eleven assets. Turnover is what was actually traded, "
                        "one way, annualised.")
    r.prose(
        "<strong>Less trading is better at every step, for both methods.</strong> "
        "Annual rebalancing gives the highest development Sharpe and the lowest "
        "turnover at the same time, at 39 trades across thirty-eight years, "
        "while daily rebalancing turns over 15 to 17% of the book a year to "
    ...
    r.section("Where the duration figures come from")
    r.prose(
        "The four Treasury holdings are constant maturity positions priced off "
        "the bootstrapped zero curve, so their modified duration falls out of "
        "the curve rather than being supplied. Each figure below is the mean of "
        "that computed series over the sample.")
    r.prose(
        "The seven funds are quoted as prices with no curve behind them, so "
        "each takes the average duration its fund company publishes. Those are "
        "current figures applied across the whole sample, which is an "
        "approximation worth naming: a fund's duration drifts as its holdings "
        "and the level of rates change, and none of these is a 1982 number.")
    r.table(pd.DataFrame([
        ("2y Treasury", "1.93", "computed from the curve"),
        ("5y Treasury", "4.45", "computed from the curve"),
        ("10y Treasury", "7.79", "computed from the curve"),
        ("30y Treasury", "14.73", "computed from the curve"),
        ("Short IG", "2.7", "VFSTX, Vanguard Short-Term Investment-Grade"),
        ("IG credit", "5.93", "FBNDX, Fidelity Investment Grade Bond"),
        ("Long IG", "11.8", "VWESX, Vanguard Long-Term Investment-Grade"),
        ("High yield", "2.9", "VWEHX, Vanguard High-Yield Corporate"),
        ("Agency MBS", "6.3", "VFIIX, Vanguard GNMA"),
        ("Municipals", "5.8", "VWITX, Vanguard Intermediate-Term Tax-Exempt"),
        ("High yield municipals", "8.2",
         "VWAHX, Vanguard High-Yield Tax-Exempt"),
        ("Aggregate index", "4.16",
         "estimated from daily price sensitivity to the US 10 year yield"),
    ], columns=["holding", "modified duration, years", "source"]).set_index(
        "holding"), compact=True, align_right=["modified duration, years"],
        caption="Fund figures are the average duration published by the fund "
                "company; Treasury figures are computed from the curve this "
                "project builds.")
    r.prose(
        "<span class=\"note\">An earlier version of this project carried a set "
        "of hardcoded durations with no source, several of which were wrong. "
        "The thirty year Treasury was quoted at 18.5 years against a computed "
        "14.73, and every fund figure was an unsourced approximation. Portfolio "
        "durations reported anywhere in this writeup use the table above.</span>")
...
def index():
    # The same three tables the phase pages render, so the summary cannot
    # drift away from the results it is summarising.
    ...
    if all(v is not None for v in TB.values()):
        ...
        for name in order:
            ...
            body += (f"<tr{cls}><th>{STRATEGY_NAME.get(name, name)}</th>"
...
.eyebrow{{font-family:"IBM Plex Mono",monospace;font-size:.72rem;letter-spacing:.16em;text-transform:uppercase;color:var(--mut);opacity:.7}}
...
<div class="wrap">
<span class="eyebrow">Gus Guenther, September 2026</span>
...
<p><b>The asset universe.</b> Eleven fixed income assets: four constant maturity
