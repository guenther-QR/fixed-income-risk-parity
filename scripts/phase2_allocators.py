...
# Per-asset round-trip cost in annualised terms, applied to turnover.
...
def max_diversification(train, rf, **kw):
    """
    Choquet's diversification ratio: weighted average vol over portfolio
    vol.
    """
    S = cov.ledoit_wolf(train)
    v = np.sqrt(np.diag(S))
...
def backtest(r, rf, alloc, edges=None, rebalance=1, label=""):
    """
    Walk-forward: fit on everything through t-1, hold through t, pay for
    trades.
    """
