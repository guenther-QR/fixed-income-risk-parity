...
class HardSwitch:
    ...
    def __call__(self, train: pd.DataFrame, rf: pd.Series | None = None) -> np.ndarray:
        ...
        # The regime label used for the decision is the last one observed
        # inside the training window - the engine has already excluded
        # anything later, so this is the most recent knowable state.
...
def _transition_row(history: pd.Series, state: str,
    ...
    """
    One row of the empirical transition matrix, estimated on the training
    window.
    """
    s = history.dropna()
    if len(s) < 2:
